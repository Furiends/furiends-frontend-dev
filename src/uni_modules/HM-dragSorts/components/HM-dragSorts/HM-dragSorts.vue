	...
	<view class="HM-drag-sort" :style="{'height': ListHeight+'px'}">
		<view class="rowBox-shadow" :style="{'height': rowHeight+'px'}" id="shadowRowBox">
			<view class="row" id="shadowRow">
				<view class="modules">
					<!-- 内容  -->
					<drag-list-item class="drag-list-item" :sortItem="row" :showDetail="showDetail" :sortIndex="index"
						@itemEditEvent="handleItemEdit(index)" @itemDelEvent="handleItemDel(index)">
					</drag-list-item>
					<!-- 拖拽图标 -->
						...
						<view class="rowBox" :style="{'height': rowHeight+'px'}">
							<view :class="'row row'+listType" :style="{'height': rowHeight+'px'}" :data-id="row.id"
								...
								<view class="modules" @tap="triggerClick(index,row)">
									<!-- 内容  -->
									<drag-list-item class="drag-list-item" :sortItem="row" :showDetail="showDetail"
										:sortIndex="index" @itemEditEvent="handleItemEdit(index)"
										@itemDelEvent="handleItemDel(index)">
									</drag-list-item>
									<!-- 拖拽图标 -->
		...
		<view :data-isapph5="isAppH5" :data-guid="guid" :data-islongtouch="isLongTouch"
			:data-isautoscroll="isAutoScroll" :data-feedbackgeneratorstate="feedbackGeneratorState"
			:data-longtouchtime="longTouchTime" :data-listheight="ListHeight" :data-rownum="list.length" id="dataView"
...
<script module="renderjs" lang="renderjs">
	import dragListItem from '@/components/drag-list-item/drag-list-item.vue'
	// APP or H5端renderjs
	export default {
		components: {
			'drag-list-item': dragListItem
		},
		data() {
			return {
				e: null,
	...
	export default {
		...
		mounted() {
			if (this.listHeight == 0) {
				...
				this.ListHeight = this.list.length*55;
			}
		...
		methods: {
			handleItemEdit(index) {
				...
				this.$emit('itemEditEvent', index);
			},
			handleItemDel(index) {
				console.log('handleItemDel', index);
				this.$emit('itemDelEvent', index);
			},
			getGuid() {
				function S4() {
			...
			initList() {
				...
				console.log("initList",JSON.parse(JSON.stringify(tmpList)));
